...
def export_feature_importance(model: XGBClassifier, preprocessor: ColumnTransformer) -> None:
    ...
    pd.DataFrame(rows).sort_values("gain", ascending=False).head(50).to_csv(
        RESULTS_DIR / "concrete_epd_feature_importance_top50.csv", index=False
    )
...
"""
    ...
    benchmark_text = ""
    benchmark_path = RESULTS_DIR / "concrete_epd_algorithm_benchmark_summary.csv"
...
"""
    ...
    custom_text = ""
    custom_path = RESULTS_DIR / "concrete_epd_carm_boost_fixed_vs_sota.csv"
...
"""
    ...
    (RESULTS_DIR / "concrete_epd_reviewer_brief.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    ).to_csv(RESULTS_DIR / "concrete_epd_dataset_summary.csv", index=False)
    (RESULTS_DIR / "concrete_epd_leakage_audit.json").write_text(
        json.dumps(
            {
    ...
    results.to_csv(RESULTS_DIR / "concrete_epd_model_results.csv", index=False)
    predictions.to_csv(RESULTS_DIR / "concrete_epd_model_predictions.csv", index=False)
    ci.to_csv(RESULTS_DIR / "concrete_epd_bootstrap_ci.csv", index=False)
    sensitivity.to_csv(RESULTS_DIR / "concrete_epd_sensitivity_results.csv", index=False)
    repeated_group.to_csv(RESULTS_DIR / "concrete_epd_repeated_group_company.csv", index=False)
    repeated_group_summary.to_csv(
        RESULTS_DIR / "concrete_epd_repeated_group_company_summary.csv", index=False
    )
    ...
    (RESULTS_DIR / "concrete_epd_run_manifest.json").write_text(
